...
run_evaluation("model_1_2", build_model_1_2, preprocessed_split, base_dir, train=True, epochs=3, batch_size=32)
...
def build_model_1_3():
    ...
    return model_1_3
    pass
run_evaluation("model_1_3", build_model_1_3, preprocessed_split, base_dir, train=True, epochs=25, batch_size=32)
answer_q_1_3 = """
               The purpose of regularization the model to avoid overfitting. Results with combination of l1 and l2 regularization (with lambda= 0.0001) is better than the l1 regularization(with lambda= 0.001). The dropout of neurons after each layer did not improve the accuracy.
               """
...
answer_q_2_1 = """
                I have implemented four 'blocks' each, consisting two 2D-convolution layers first ehich is followed by 2D maxpooling and finally dropout. 
                The first layer Conv2D uses it's kernel to extract all the spatial features from the data. If we see carefully, ReLU does not activate all the neurons at the same time.
                Hence ReLU performed relatively fast as compared to other things.
                The main idea is to use multiple convolutional layers to learn patterns. This learning is done at all the different levels of abstraction. 
                Then MaxPooling was used which does two things. It increases the translation invariance and reduces the resolution, which are both necessary. 
                Performance wise, the convolution layers performed very good. 
                Unfortunately, there the model was overfitting. Hence, dropout of 0.6, 0.3 and 0.05 was observed. The best performance found is on lambda 0.3. 
                The accuracy is very high with ~94%. Another goo thing to notice is that there is very small difference in accuracy and val_accuracy. 
                With these considerations we cannot conclude that the model is overfitting and hence, the model was finalized.
               """
...
def augmentation():
    ...
    new_X_val= rgb2gray(X_val)
    new_X_test= rgb2gray(X_test)

    augmented_split = new_X_train, new_X_val, new_X_test, y_train, y_val, y_test

    rotate= np.zeros(len(X_train))

    return augmented_split

augmented_split = augmentation()[0], augmentation()[1], augmentation()[3], augmentation()[4]

def build_model_2_2():
    model_2_2 = models.Sequential()
    model_2_2.add(layers.Conv2D(16, (3, 3), activation='relu', padding='same', input_shape=(32, 32, 3)))
    model_2_2.add(layers.Conv2D(16, (3, 3), activation='relu', padding='same'))
    model_2_2.add(layers.MaxPooling2D((2, 2)))
    model_2_2.add(layers.Dropout(0.3))
...
answer_q_2_2 = """
               After the Visual inspection, I saw that some of the images are slightly tilted over the axis. Some images were also shifted towards the left or right. 
               This means alignment is not correct and to fix it, the images are being rotated and shifted for better performance and results. 
               Although this is important, it did not seem to reflect significantly on the accuracy of the model made. 
               The accuracy score is coming out to be 0.942 and the val accuracy is 0.946, again with low difference which is good.
               When I compared it with the preprocessed data as explained above, it did improve the score a bit, but was not as significant as expected. 
               The new model made is essentially the same, as the base of the previous model is used with little change in shape.
               """
...
test_score_3_1, test_accuracy_3_1 = model_2_1.evaluate(rgb2gray(X_test), y_test, batch_size=32)
y_pred = model_2_1.predict(rgb2gray(X_test))
...
answer_q_3_1 = """By running the model multiple times, I observed that the images which have more than one digit are the problem area and are usually misclassfied. 
                  Since the model I made is designed only to classify single digits (per image), it performs poorly when asked to classify multiple digits in a single image input. 
                  Another problematic area was classification of 8. Whenever the model encounters the class of 8, it is usually misclassified as 6 or 0. 
                  Some misclassified images were because of the extremely poor quality of images. These were even hard to classify by human eye.
                  Statistically, the highest number of missclassifications found was between 6 and 0 and between 3 and 0. 
                  Everything analyzed together, we can say that 0 is often being misclassified as other digit."""
...
f, img= plt.subplots(1, 1, sharey=True)
img.imshow(X_test[7])
img_tensor = rgb2gray(X_test)[7]
...
images_per_row = 16
layer_names = []
for layer in model_2_1.layers[:15]:
...
answer_q_3_2= """To explain this, let's take one instance and consider the 8th image. 
                The first layer of the model, it learns about the edges of the digit and the very basic structure of inside of the digit. 
                Towards the sixth layer, the output is much more detailed and precise. 
                The model also learns that the structures that were being ignored in the initial layers are also important and hence prominent in the output. 
                This makes the final output, more noisy than the first layer."""
...
answer_q_4_1 = """Initially when the first 10 layers are unfreezed, I found that the model performs well.
                  Later when no layers are freezed the accuracy dropped to 0.18, which was very low and bad.
                  When all the layers are freezed, the accuracy came out to be 0.1908. Hence, I then tried to freeze different convolution layers of the base. 
                  The accuracy of final model was much better than the previous freezed attempts."""
...
model_4_123= load_model_from_file(base_dir, "model_4_1")
...
def store_embeddings():
    ...
    shout("Saving to file", verbose= 1)
...
def generate_pipeline():
    ...
    """
    return Pipeline(steps=[('classifier', DecisionTreeClassifier())])
...
def evaluation_4_2(X_train, y_train, X_test, y_test):
    ...
    print("Hence, the accuracy score is: ", evaluate_pipeline(X_train_emb, X_test_emb))

# store_embeddings()
# evaluation_4_2(X_train, y_train, X_test, y_test)

answer_q_4_2 = """
               Let's look closely how the store embedding function works. It first fetches all the intermediate layers. 
               After that the embeddings of the test are called and the training data is generated.
               This data is then stored in the external file system. Meanwhile, using the 'decision tree classifier' a pipeline is generated.
               The embeddings of the training and testing data (i.e. X_test and X_train) are loaded with the help of helper functions.
               These values are passed to evaluate the ealier created pipeline. 
               The accuracy score of pipeline after evaluation was not very good, which is because it's the accuracy score with outputs generated from 
               intermediate layers.
               """
